...
| Seed coverage | Partial | The Classic-small, Journey-medium, and Classic-large release matrix generates, reloads, saves, and reloads again across Corruption and Crimson. Exploratory seeds have also exposed candidate-exhaustion failures in the guaranteed mine and required Evil landmark planners, outside the Torch God pass. | Add a bounded ordinary-seed soak test, generation-time limits, automatic failure artifact retention, and guaranteed fallback sites for the mine and required landmarks. |
...
| Torch God temple | Implemented | One late-planned underground temple connects to a natural cave, selects one of four layouts and ten biome palettes, retains a thick brick shell and unsafe walls, and furnishes a central altar. Exactly 100 lit themed torches surround an empty socket; the altar chest contains the one ordinary Torch needed to cross Terraria's activation threshold. Candidate sites retain a dry buffer, and both reload stages recount the live tiles after liquid settling. | Add a live-player activation and arena-combat test, then add damaged and overgrown layout variants without changing the exact torch budget. |

...
| Seed coverage | Partial | The Classic-small, Journey-medium, and Classic-large release matrix generates, reloads, saves, and reloads again across Corruption and Crimson. A bounded twelve-world soak rotates sizes and modes, retains per-world failure artifacts, and exercises obstacle-aware mine routing plus deterministic mine and Evil-landmark fallbacks. | Add longer rotating and cross-mod campaigns, explicit generation-time budgets, and automatic visual comparisons. |
...
| Mountain ranges and valleys | Implemented | Ground-connected ranges vary between Highland, Alpine, and Sky-piercing altitude families. Their skins and suspended chamber ledges inherit Forest, Snow, Desert, Jungle, Corruption, or Crimson material families. Four interior grammars provide short turning cave legs, broad organically walled chambers, open-background pockets, shafts, climb aids, thick natural spawn shelves, furnished vignettes, pots, rubble, living-root vine curtains, protected ponds or lakes, optional cloud belts, saddle valleys, and structural bridges with clear endpoint corridors. Seeded stone bridges can carry enough complete tombstones to create the Graveyard biome. | Add stronger inherited-biome route families, summit ruins, natural arches, connected waterfalls, and more bridge damage states. |
...
| Guaranteed surface mine | Implemented | The visible headframe connects to a cyclic rail graph with work, mountain, flooded, collapsed, and sealed evil districts. Rail tunnels keep six guaranteed clearance tiles while correlated macro profiles create wider and taller cavern sections. Downhill connectors use open-ended gravity drops, jump motifs use real ramp frames, and terminal workings use bouncy bumpers. | Add an automated live-cart trajectory test, more damage states, drainage links, and secondary surface exits. |
| Torch God temple | Implemented | One late-planned underground temple connects to a natural cave, selects one of four layouts and ten biome palettes, and keeps its thick brick mass at the perimeter, bottom floor, short edge galleries, and compact altar. Matching construction-wall panels inherit the biome material, while correlated unsafe borders and seams prevent NPC housing. The broad central chamber is at least 82% collision-open, contains at least 72 platform tiles, and retains twelve wide movement rows. Exactly 100 lit themed torches surround an empty socket; the altar chest contains the one ordinary Torch needed to cross Terraria's activation threshold. Candidate sites retain a dry buffer, and both reload stages recount the live tiles after liquid settling. | Pin visual seeds for every non-Desert palette, add a live-player activation and arena-combat test, then add damaged and overgrown variants without weakening the open-arena, wall-composition, or exact-torch contracts. |
...
| Ores, gems, chests, traps, pots, and loot tables | Vanilla | Vanilla distribution and rewards remain authoritative. Authored landmarks and mine districts are loot-neutral; the Torch God altar chest is the narrow exception and contains exactly one ordinary Torch required by its vanilla event setup. | Add rewards only with world-size budgets and checks that prevent duplicated progression items or abnormal resource density. |
| Trees, plants, vines, cactus, coral, and ambient biome decoration | Partial | Terraria owns region-wide vegetation. Vanilla Worlds Overhauled adds contextual debris and dense mountain curtains made from Terraria's growing vine species, each attached to a compatible living root, including humidity-biased growth around water bodies. Mountain caves also receive pots, rubble, torches, and climb aids. | Add planting passes for each biome-specific terrain family after their route and structure bounds become stable. |

			...
			if (!TryReserve(plan, request, request.Required, respectStructureMap: !request.Required, out BuildTerrace terrace, out string failure)) {
				if (request.Required) {
					throw new InvalidOperationException(
						"Vanilla Worlds Overhauled could not preserve the required spawn building terrace. " + failure);
				}
			}
			else {
		...
		for (int index = manifest.Terraces.Count - 1; index >= 0; index--) {
			...
			if (!TryMeasureSurface(left, right, out int surfaceY, out int relief)) {
				if (terrace.SpawnTerrace) {
					throw new InvalidOperationException("Vanilla Worlds Overhauled lost the required spawn building terrace before stabilization.");
				}
			...
			if (!TileEditor.IsSafeForTerrainFeature(preflight)) {
				if (terrace.SpawnTerrace) {
					throw new InvalidOperationException($"Vanilla Worlds Overhauled could not repair the spawn building terrace at x={left} without touching an object.");
				}
				manifest.Terraces.RemoveAt(index);
				continue;
			}

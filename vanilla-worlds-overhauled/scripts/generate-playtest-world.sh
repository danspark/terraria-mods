...
build_save_dir="${VANILLA_WORLDS_OVERHAULED_BUILD_SAVE_DIR:-$mod_dir/.playtest/build-save}"
source_package="$build_save_dir/Mods/VanillaWorldsOverhauled.tmod"
...
world_name="Vanilla Worlds Overhauled ${world_size^} ${world_mode^}"
...
cp -f "$source_package" "$playtest_dir/Mods/VanillaWorldsOverhauled.tmod"
printf '[\n  "VanillaWorldsOverhauled"\n]\n' > "$playtest_dir/Mods/enabled.json"
...
if ! grep -q 'Vanilla Worlds Overhauled validation passed' "$generation_log"; then
	printf 'The world exists, but Vanilla Worlds Overhauled validation was not recorded.\n' >&2
	exit 1
fi
...
if ! grep -Eq 'Loaded Vanilla Worlds Overhauled manifest v6: landmarks=11; mountains=[1-9][0-9]*; bridges=[1-9][0-9]*; forestLakeBridges=[0-9]+; mountainWaters=[1-9][0-9]*; skyHighlands=[1-9][0-9]*; mine=present; validation=valid=True' "$reload_log"; then
...
if ! grep -Eq 'Loaded Vanilla Worlds Overhauled manifest v6: landmarks=11; mountains=[1-9][0-9]*; bridges=[1-9][0-9]*; forestLakeBridges=[0-9]+; mountainWaters=[1-9][0-9]*; skyHighlands=[1-9][0-9]*; mine=present; validation=valid=True' "$persistence_log"; then

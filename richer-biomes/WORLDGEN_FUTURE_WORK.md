...
| Area | Status | Current coverage | Future work |
| --- | --- | --- | --- |
| Biome landmarks | Implemented | Required biome categories receive connected ground bays and upper rooms with two-tile posts and ceilings, filled correctly sloped gables, diagonal platform stairs and landings, strict interior wall fields, themed vanilla furniture, tapered foundations, and rounded open side arches. Forest landmarks use ordinary Wood Blocks and Wooden Beams. Final tile validation checks real object footprints and slope state; landmarks author no doors and must fail Terraria's real NPC-housing query. | Add building-type silhouettes beyond the shared connected-room grammar, stronger biome-specific aging, and partially ruined variants without weakening traversal or anti-housing checks. |

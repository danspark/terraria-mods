...
| Transition zones | Partial | Coast elevations blend into the world edge. Mountain material repair follows the biome support found after vanilla shaping. | Add planned forest-snow, forest-desert, forest-jungle, and ordinary-evil transition regions with gradual material changes and protected paths. |
...
| Visual regression testing | Partial | Fixed seeds have strict tile validation and independent full-map and feature-crop inspection. | Move the inspector and crop renderer into the repository so CI can compare silhouettes and authored districts. |
| Seed coverage | Partial | The test matrix covers one fixed Classic small world, one Journey medium world, and the reference Classic large world. | Add a bounded ordinary-seed soak test, generation-time limits, and failure artifact retention across world sizes and evil variants. |
...
| Mountain ranges and valleys | Implemented | Ground-connected ranges reach Space and contain interior routes, chimneys, cloud belts, saddle valleys, and structural bridges. | Add biome-specific route families, summit landmarks, natural arches, waterfalls, and more bridge damage states. |
...
| Floating highlands | Implemented | World-size highlands contain top, interior, and underside routes, chambers, vertical shafts, lakes, satellites, cloud belts, and mountain connections. | Integrate more district variants and test live sky combat and fishing. Vanilla Floating Island loot and houses remain under vanilla ownership. |
...
| Area | Status | Current coverage | Future work |
| --- | --- | --- | --- |
| Biome landmarks | Implemented | Required biome categories receive styled, furnished structures with mixed walls, thick foundations, windows, bounded platform openings, and final tile validation. | Add more layout families and biome-specific furniture styles without weakening the current structural checks. |
...
| Trees, plants, vines, cactus, coral, and ambient biome decoration | Vanilla | Terraria owns vegetation and most natural decoration. Richer Biomes adds sparse biome-aware debris piles outside quiet areas. | Add planting passes for each biome-specific terrain family after their route and structure bounds become stable. |
...
2. Add transition-zone plans and local watershed records to `WorldPlan`.
3. Replace the biome-neutral cave shape with typed cave grammars, starting with Forest, Snow, and Jungle.
